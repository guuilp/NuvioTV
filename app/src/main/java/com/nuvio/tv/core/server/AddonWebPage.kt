...
<title>NuvioTV - Manage from Phone</title>
  ...
  catalogs.forEach(function(catalog, i) {
    ...
    li.innerHTML =
      ...
      '<div class="catalog-info">' +
        '<div class="catalog-name">' + escapeHtml(isCollection ? catalog.catalogName : typeDisplay) +
          (catalog.isDisabled ? '<span class="badge-disabled">Hidden</span>' : '') +
          (isCollection ? '<span style="display:inline-block;font-size:0.6rem;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:rgba(180,160,255,0.9);border:1px solid rgba(180,160,255,0.35);padding:0.12rem 0.45rem;border-radius:100px;margin-left:0.5rem;vertical-align:middle">Collection</span>' : '') +
        '</div>' +
        '<div class="catalog-meta">' + escapeHtml(catalog.addonName) + '</div>' +
      '</div>' +
      '<div class="addon-actions">' +
        '<button class="' + toggleClass + '" onclick="toggleCatalog(' + i + ')">' +
          (catalog.isDisabled ? 'Show' : 'Hide') +
        '</button>' +
      '</div>';
...
function showSuccessStatus() {
  ...
  content.innerHTML =
    ...
    '<div class="status-message">Your changes have been applied on the TV.</div>';
